...
fn main() -> std::io::Result<()> {
    ...
    for _ in 0..reader.read() {
        ...
        let h: i64 = reader.read();
        let mut a = Vec::with_capacity(n);
        for _ in 0..n {
            a.push(reader.read::<i64>());
        }
        a.sort();
        fast_writeln!("{}", dfs(&a, 0, h, 2, 1));
    }
...
fn dfs(a: &Vec<i64>, idx: usize, h: i64, green: i64, blue: i64) -> i64 {
    if idx == a.len() {
        return 0;
    }
    if h > a[idx] {
        return 1 + dfs(a, idx + 1, h + a[idx] / 2, green, blue);
    } else {
        let mut t = 0;
        if green > 0 {
            t = t.max(dfs(a, idx, h * 2, green - 1, blue));
        }
        if blue > 0 {
            t = t.max(dfs(a, idx, h * 3, green, blue - 1));
        }
        return t;
    }
}

...
fn main() -> std::io::Result<()> {
    ...
    for _ in 0..reader.read() {
        ...
        arr.sort_unstable();
        let mut ans = (arr[n - 1] - arr[2] + 1) / 2;
        let mut p = 1;
        for i in 3..n {
            let c = (arr[n - 1] - arr[i] + 1) / 2;
            while p + 1 < i {
                let a = (arr[p - 1] - arr[0] + 1) / 2;
                let b = (arr[i - 1] - arr[p] + 1) / 2;
                let x = (arr[p] - arr[0] + 1) / 2;
                let y = (arr[i - 1] - arr[p + 1] + 1) / 2;
                if a.max(b) >= x.max(y) {
                    p += 1;
                    ans = ans.min(c.max(x).max(y));
                } else {
                    ans = ans.min(c.max(a).max(b));
                    break;
                }
            }
        }
